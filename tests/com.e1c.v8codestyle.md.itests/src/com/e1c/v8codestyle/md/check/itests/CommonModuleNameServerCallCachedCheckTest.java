 ...
 */
public class CommonModuleNameServerCallCachedCheckTest
    extends CheckTestBase
{

    private static final String CHECK_ID = "server-call-cached";
    ...
    public void testCommonModuleNameServerCallWithoutPostfixCorrect() throws Exception
    {
        ...
        assertEquals("Common module should end with ServerCall postfix", marker.getMessage());
    }

    @Test
    public void testCommonModuleNameServerCallWithoutPostfixCached() throws Exception
    {
        IDtProject dtProject = openProjectAndWaitForValidationFinish(PROJECT_NAME);
        assertNotNull(dtProject);
        ...
        updateCommonModule(dtProject, MODULE_DEFAULT_FQN, CommonModuleTypes.CLIENT, ReturnValuesReuse.DURING_REQUEST,
            fqn);

        long id = getTopObjectIdByFqn(fqn, dtProject);
        Marker marker = getFirstMarker(CHECK_ID, id, dtProject);
        assertEquals("Common module should end with Cached postfix", marker.getMessage());
    }
