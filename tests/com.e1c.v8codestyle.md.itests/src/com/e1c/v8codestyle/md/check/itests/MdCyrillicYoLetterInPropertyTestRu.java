 ...
 */
public class MdCyrillicYoLetterInPropertyTestRu
    extends CheckTestBase
{

    private static final String CHECK_ID = "md-cyrillic-yo-letter"; //$NON-NLS-1$
    private static final String PROJECT_NAME = "MdCyrillicYoLetterInPropertyRu";

    /**
     * Test MD-Object has cyryllic yo letter for Russian default lang
     *
    ...
    public void testNegativeTest() throws Exception
    {
        ...
        id = getFirstAttributeId_Catalog(dtProject, "Catalog.NegativeCatalog_Attribute_Name");
        marker = getFirstMarker(CHECK_ID, id, dtProject);
        assertNotNull(marker);

        id = getFirstAttributeId_Catalog(dtProject, "Catalog.NegativeCatalog_Attribute_Comment");
        marker = getFirstMarker(CHECK_ID, id, dtProject);
        assertNotNull(marker);
        ...
        id = getFirstTabularId_Catalog(dtProject, "Catalog.NegativeCatalog_TabularSection_Name");
        marker = getFirstMarker(CHECK_ID, id, dtProject);
        assertNotNull(marker);

        id = getFirstTabularId_Catalog(dtProject, "Catalog.NegativeCatalog_TabularSection_Comment");
        marker = getFirstMarker(CHECK_ID, id, dtProject);
        assertNotNull(marker);
     ...
     * @return
     */
    private long getFirstAttributeId_Document(IDtProject dtProject, String name)
    {
        long id;
        BmObject object = (BmObject)getTopObjectByFqn(name, dtProject);
        assertTrue(object instanceof Document);
        Document document = (Document)object;
        id = ((IBmObject)document.getAttributes().get(0)).bmGetId();
        return id;
    }
