...
/**
 * Checks that only standard role has right: {@code EXTERNAL_CONNECTION} for configuration root.
 *
    ...
    extends RoleRightSetCheck
{
    ...
    private static final List<String> NAMES_DEFAULT =
        List.of("ЗапускВнешнегоСоединения", "StartExternalConnection", "АдминистраторСистемы", "SystemAdministrator"); //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$ //$NON-NLS-4$
    ...
    protected void configureCheck(CheckConfigurer builder)
    {
        ...
        builder.extension(new ExcludeRoleByNameListExtension(NAMES_DEFAULT, bmModelManager))
            .title(Messages.StartExternalConnectionRight_title)
            .description(Messages.StartExternalConnectionRight_description);
    }

    @Override
    protected RightName getRightName()
    {
        return RightName.EXTERNAL_CONNECTION;
    }
