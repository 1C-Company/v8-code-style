...
@QuickFix(checkId = "configuration-brief-information", supplierId = CorePlugin.PLUGIN_ID)
public class ConfigurationBriefInformationFix
    extends SingleVariantModelBasicFix<Configuration>
{
    ...
    protected void configureFix(SingleVariantModelBasicFix.FixConfigurer configurer)
    {
        ...
        configurer.description("Replace brief information with synonym");
    }
    ...
    protected void applyChanges(Configuration object, EStructuralFeature feature, BasicModelFixContext context,
        IFixSession session)
    {
        object.getBriefInformation().clear();
        for (Entry<String, String> entry : object.getSynonym())
        {
            object.getBriefInformation().put(entry.getKey(), entry.getValue());
        }
    }
}
